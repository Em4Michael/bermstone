    ...
    <div className="min-h-screen bg-[#FAFBFF]">
      ...
      <div className="relative bg-animated-gradient noise overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 rounded-full opacity-20 pointer-events-none"
          style={{ background: 'radial-gradient(circle, #C9A84C, transparent)', filter: 'blur(60px)' }} />
      ...
      <div className="sticky top-20 z-30 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex flex-col md:flex-row gap-2">
            ...
            {hasActive && (
              <button onClick={clear} className="flex items-center gap-1.5 px-3 py-2.5 text-sm text-red-500 hover:text-red-700 font-medium">
                <X size={13} />Clear
              </button>
            )}
          </div>
          ...
          {showFilters && (
            <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-3 animate-fade-down">
              ...
              <div>
                ...
                <select value={filters.bedrooms} onChange={e => set('bedrooms', e.target.value)} className="form-input text-sm">
              ...
              <div>
                ...
                <select value={filters.maxGuests} onChange={e => set('maxGuests', e.target.value)} className="form-input text-sm">
                  <option value="">Any</option>
              ...
              <div className="col-span-2 md:col-span-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {AMENITIES.map(a => {
                    const active = filters.amenities.split(',').filter(Boolean).includes(a);
                    return (
                      <button key={a} type="button" onClick={() => toggleAmenity(a)}
                        className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-all',
                          active ? 'bg-[#0B1F3A] text-white border-[#0B1F3A]' : 'border-slate-200 text-slate-600 hover:border-[#0B1F3A]')}>
                        {a}
                      </button>
                    );
                  })}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          <span className="text-sm text-slate-400 self-center mr-1">Browse by city:</span>
          {CITIES.map(c => (
            <button key={c} onClick={() => set('city', filters.city === c ? '' : c)}
              className={cn('px-4 py-1.5 rounded-full text-xs font-medium border transition-all duration-200',
                filters.city === c
        ...
        {!loading && (
          <p className="text-sm text-slate-400 mb-6">
            Showing <strong className="text-[#0B1F3A]">{properties.length}</strong>
            {total > properties.length && <> of <strong className="text-[#0B1F3A]">{total}</strong></>} apartments
        ...
        {loading ? (
          ...
          <div className="text-center py-28">
            ...
            <h3 className="font-display text-2xl font-semibold text-[#0B1F3A] mb-2">No apartments found</h3>
            <p className="text-slate-400 mb-6">
              {hasActive ? 'Try adjusting your filters or clear them to see all apartments.' : 'No apartments available yet.'}
            </p>
            {hasActive && <button onClick={clear} className="btn-primary">Clear All Filters</button>}
          </div>
        ) : (
          <>
