...
import { User, Mail, Phone, Shield, CalendarCheck, TrendingUp, LogOut } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const ROLE_INFO = {
    guest:    { label: 'Guest',           color: 'bg-blue-100 text-blue-700',   icon: '🏠', desc: 'Browse and book premium shortlet apartments.' },
    owner:    { label: 'Property Owner',  color: 'bg-green-100 text-green-700', icon: '🏢', desc: 'List and manage your properties with Bermstone.' },
    investor: { label: 'Investor',        color: 'bg-yellow-100 text-yellow-700', icon: '📈', desc: 'Access and invest in high-yield real estate projects.' },
    admin:    { label: 'Administrator',   color: 'bg-red-100 text-red-700',     icon: '⚙️', desc: 'Full administrative access to the platform.' },
  };
    ...
    <div className="min-h-screen bg-[#FAFBFF] pt-20">
      ...
      <div className="bg-[#0B1F3A] pt-14 pb-20 px-4">
        <div className="max-w-3xl mx-auto flex items-center gap-6">
          ...
          <div>
            <h1 className="font-display text-3xl font-semibold text-white">{user.firstName} {user.lastName}</h1>
            <p className="text-blue-200 text-sm mt-1">{user.email}</p>
            <span className={`badge mt-2 ${roleInfo.color}`}>{roleInfo.icon} {roleInfo.label}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 -mt-8 pb-16 space-y-6">
        ...
        <div className="card p-6">
          <h2 className="font-display text-xl font-semibold text-[#0B1F3A] mb-5">Account Details</h2>
          <div className="space-y-4">
            <div className="flex items-center gap-3 py-3 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#0B1F3A]">{user.firstName} {user.lastName}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 py-3 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#0B1F3A]">{user.email}</p>
              </div>
            </div>
            {user.phone && (
              <div className="flex items-center gap-3 py-3 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-[#0B1F3A]">{user.phone}</p>
                </div>
              </div>
            )}
            <div className="flex items-center gap-3 py-3">
              ...
              <div>
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Account Role</p>
                <p className="text-sm font-medium text-[#0B1F3A]">{roleInfo.label}</p>
        ...
        <div className="card p-6">
          <h2 className="font-display text-xl font-semibold text-[#0B1F3A] mb-5">Quick Actions</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link href="/properties"
              ...
              <CalendarCheck size={20} className="text-[#3B9EE0] group-hover:text-[#1E5FBE]" />
              <div>
                <p className="text-sm font-medium text-[#0B1F3A]">Browse Shortlets</p>
            ...
            {(user.role === 'investor' || user.role === 'admin') && (
              <Link href="/investments"
                ...
                <div>
                  <p className="text-sm font-medium text-[#0B1F3A]">View Investments</p>
            ...
            {user.role === 'owner' && (
              <Link href="/owner-support"
                className="flex items-center gap-3 p-4 border border-slate-100 rounded-xl hover:border-[#1E5FBE] hover:bg-blue-50 transition-all group">
                <Shield size={20} className="text-[#3B9EE0] group-hover:text-[#1E5FBE]" />
                <div>
                  <p className="text-sm font-medium text-[#0B1F3A]">Owner Support</p>
            ...
            {isAdmin && (
              <Link href="/admin"
                ...
                <div>
                  <p className="text-sm font-medium text-[#0B1F3A]">Admin Dashboard</p>
