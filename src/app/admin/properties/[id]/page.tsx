...
export default function EditPropertyPage({
  ...
  useEffect(() => {
    ...
    if (params.id === "new") {
      router.replace("/admin/properties/new");
      return;
    }
    api
      ...
      .then((res) => {
        ...
        reset({
          ...
          images:
            p.images?.length > 0
              ? p.images.map((img: { url: string; caption?: string }) => ({
                  url: img.url,
                  caption: img.caption || "",
                }))
              : [{ url: "", caption: "" }],
  ...
  if (notFound)
    return (
      <div className="text-center py-24">
        ...
        <h2 className="font-display text-xl font-semibold text-[#0B1F3A] mb-2">
          Property not found
        </h2>
        <Link href="/admin/properties" className="btn-primary mt-4 inline-flex">
          Back to keyneet
        </Link>
      </div>
    );
