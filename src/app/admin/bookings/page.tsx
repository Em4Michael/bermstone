...
export default function AdminBookingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [bookRes, propRes] = await Promise.all([
        ...
        api.get("/keyneet", { params: { limit: 100, admin: "true" } }),
      ]);
      setBookings(bookRes.data.data || []);
      setkeyneet(propRes.data.data || []);
    } catch {
      setBookings([]);
    } finally {
      setLoading(false);
  ...
  const selectedProperty = keyneet.find((p) => p._id === addForm.property);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={load}
            className="p-2.5 text-slate-400 hover:text-[#1E5FBE] border border-slate-200 rounded-xl transition-colors"
          >
            <RefreshCw size={16} />
          </button>
          <button
          ...
          <form onSubmit={handleAdd} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, property: e.target.value }))
                  }
                  required
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, guests: e.target.value }))
                  }
                  className="form-input text-sm"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, checkIn: e.target.value }))
                  }
                  className="form-input text-sm"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, checkOut: e.target.value }))
                  }
                  className="form-input text-sm"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, paymentStatus: e.target.value }))
                  }
                  className="form-input text-sm"
                >
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, specialRequests: e.target.value }))
                }
                rows={2}
          ...
          <div className="bg-white rounded-2xl p-6 shadow-2xl w-full max-w-md">
            ...
            <div className="bg-slate-50 rounded-xl p-4 mb-4 text-sm space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="font-mono font-semibold text-[#1E5FBE]">
                  {payModal.bookingReference}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Guest</span>
                <span className="font-medium">
                  {payModal.guestInfo.firstName} {payModal.guestInfo.lastName}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Amount</span>
                <span className="font-semibold text-green-600">
                  {formatCurrency(payModal.totalAmount, payModal.currency)}
                </span>
              </div>
            </div>
            <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {["", "pending", "confirmed", "cancelled", "completed"].map((s) => (
            <button
              ...
              onClick={() => setFilter(s)}
              className={cn(
                "px-3 py-2.5 rounded-xl border text-sm capitalize transition-all",
                filter === s
              ...
              {filtered.map((b) => (
                ...
                >
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs text-[#1E5FBE] font-semibold">
                      {b.bookingReference}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-medium text-[#0B1F3A] whitespace-nowrap">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={cn(
                        "badge text-xs",
                        PAYMENT_COLORS[b.paymentStatus] || "bg-slate-100",
                      )}
                    >
